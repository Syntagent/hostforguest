...
export const ACCOMMODATION_CHECKLIST: AccommodationChecklistDefinition[] = [
  ...
  { id: "amenities", label: "Amenities", prompt: "Confirm amenities. Tap common options below or type anything missing." },
  { id: "services", label: "Services", prompt: "Confirm services you can actually provide or arrange for guests." },
  { id: "specialties", label: "Local specialties", prompt: "Confirm local topics where you can genuinely help guests." },
  { id: "languages", label: "Languages", prompt: "Confirm languages you can comfortably use with guests." },
...
export const CHECKLIST_EDIT_TARGETS: Partial<Record<string, string>> = {
  property_name: "stay-property-details",
  property_type: "stay-property-details",
  capacity: "stay-capacity",
  location_core: "stay-location",
  gps: "stay-location",
  location_story: "stay-guest-voice",
  amenities: "stay-amenities",
  services: "stay-amenities",
  specialties: "stay-amenities",
  languages: "stay-amenities",
  welcome_message: "stay-guest-voice",
  gallery: "stay-photos-rules",
  rules: "stay-photos-rules",
};
...
function preserveIncompleteStatus(status: AccommodationChecklistStatus | undefined): AccommodationChecklistStatus | null {
...
export function buildAccommodationChecklist(
  ...
  return ACCOMMODATION_CHECKLIST.map((definition) => {
    ...
    const isDone = itemIsDone(definition.id, snapshot);
    const preservedIncompleteStatus = preserveIncompleteStatus(previous?.status);
    const status: AccommodationChecklistStatus = isDone ? "done" : preservedIncompleteStatus ?? "missing";
    return {
      id: definition.id,
