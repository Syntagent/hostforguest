...
function CompactStatCard({ stat }: { stat: DashboardStatsCard }) {
  ...
  const className =
    "rounded-2xl border border-border bg-card p-4 shadow-sm transition hover:-translate-y-0.5 hover:bg-muted/40 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-ring";

  if (stat.href) {
    return (
      <Link href={stat.href} className={className} aria-label={`${stat.title}: ${stat.value}. Open section`}>
        {content}
      </Link>
    );
  }
...
export const OverviewTab: React.FC<{
  ...
  isLoading?: boolean;
}> = ({ hostName, statsCards, guestGroups, realtimeUpdates, onRefresh, accommodationInfo, isLoading }) => {
        ...
        <div className="min-w-0">
          ...
          <div className="mt-3 sm:max-w-xl">
            <div className="mb-1 flex items-center justify-between gap-3">
              <label
                htmlFor="active-property-selector"
              ...
              <Link
                ...
                className="inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium text-primary transition hover:bg-primary/10 focus:outline-none focus:ring-2 focus:ring-ring"
              >
            ...
            <select
              id="active-property-selector"
              value="current"
              onChange={() => undefined}
              className="h-11 w-full rounded-2xl border border-border bg-background px-3 text-sm font-medium text-foreground shadow-sm outline-none transition hover:bg-muted/60 focus:ring-2 focus:ring-ring"
            >
        ...
        <Card className="border-border shadow-sm">
          ...
          <CardContent className="space-y-3">
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground">Create the guest-facing handout when the profile is ready.</p>
            </Link>
          </CardContent>
        </Card>
      </div>
