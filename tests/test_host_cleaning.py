...
@pytest.mark.asyncio
async def test_cleaning_endpoints_require_session():
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        r = await client.get("/api/v1/cleaning/providers")
        assert r.status_code == 401


@pytest.mark.asyncio
async def test_cleaning_happy_path():
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        pr = await client.post("/api/v1/partners/", json=partner_body)
        assert pr.status_code == 201, pr.text
        partner_id = pr.json()["id"]
        up = await client.put(f"/api/v1/partners/{partner_id}", json={"status": "active"})
        ...
        prov = await client.get("/api/v1/cleaning/providers", headers=headers)
        ...
        mine = await client.get("/api/v1/cleaning/my-cleaners", headers=headers)
