...
async def _load_db_snapshot(db: AsyncSession, host: Host) -> Dict[str, Any]:
    ...
    return {key: snapshot[key] for key in VOICE_CONTEXT_FIELDS if key in snapshot}


def _merge_snapshot(db_snapshot: Dict[str, Any], client_snapshot: Dict[str, Any]) -> Dict[str, Any]:
...
async def accommodation_voice_ingest(
    ...
    snapshot = _merge_snapshot(db_snapshot, request.accommodation_snapshot)
    agent_context = request.accommodation_snapshot.get("_agent_context")
    ...
    metadata = {**metadata, "transcript": transcript, "ingested_via": "voice"}
