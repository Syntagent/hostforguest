...
Runs weekly content updates from Croatian tourism sources and notifies hosts
...
async def log_task_execution(task_name: str, results: Dict[str, Any]):
    """
    Log task results for local monitoring and operations.
    ...
    """
    try:
        logger.info(f"CONTENT_TASK_LOG: {task_name} - {results}")
    except Exception as e:
        logger.error(f"Error logging task execution: {e}")


async def update_task_status(task_id: str, status: str, notes: str = ""):
    """
    Log a scheduler task status update.
    ...
    """
    try:
        logger.info(f"CONTENT_STATUS_UPDATE: Task {task_id} -> {status}: {notes}")
    except Exception as e:
...
async def main_weekly_scraping():
    ...
    try:
        ...
        await log_task_execution("weekly_content_scraping", results)
        return results
    except Exception as e:
        logger.error(f"Error in main weekly scraping: {e}")
        await log_task_execution("weekly_content_scraping", {"error": str(e), "success": False})
        raise
...
async def main_daily_health_check():
    ...
    try:
        ...
        await log_task_execution("daily_health_check", results)
        return results
    except Exception as e:
        logger.error(f"Error in main daily health check: {e}")
        await log_task_execution("daily_health_check", {"error": str(e), "success": False})
        raise
