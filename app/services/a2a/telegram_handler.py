...
def _build_webhook_payload(url: str) -> dict:
    """Build setWebhook payload, including secret_token if configured."""
...
class TelegramA2AHandler:
    ...
    async def handle_update(self, update: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if text.startswith("/start"):
            ...
            result = await self.orchestrator.handle_start(
                text,
                user_id=user_id,
                telegram_id=int(telegram_id) if telegram_id else None,
            )
            reply = result.get("response") or host_welcome_message_hr()
            await self.send_message(chat_id, reply)
            return {
                "ok": True,
                "action": result.get("action", "welcome"),
    ...
    try:
        async with httpx.AsyncClient(timeout=20.0) as client:
            resp = await client.post(
                ...
                json=_build_webhook_payload(url),
            )
            data = resp.json()
            if data.get("ok"):
