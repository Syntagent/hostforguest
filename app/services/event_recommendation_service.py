...
_GUEST_EVENT_REC_INTERNAL_REC_KEYS = frozenset(
    {
        "id",
        "scores",
        "relevance_score",
...
_GUEST_EVENT_REC_INTERNAL_PERSONALIZATION_KEYS = frozenset(
    {"keyword_count", "preferred_radius_km", "mobility_limited"}
)


def sanitize_event_recommendations_for_guest(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Strip internal scoring metadata from guest-facing event recommendation payloads."""
    from app.services.host_offerings_for_guest import scrub_contact_from_text, _scrub_safe_value

    out = dict(payload)
    out.pop("total_candidates", None)
    if out.get("city"):
        out["city"] = scrub_contact_from_text(str(out["city"]))
    personalization = out.get("personalization")
    if isinstance(personalization, dict):
        pers = dict(personalization)
        for list_key in ("interests", "feed_cities"):
            if pers.get(list_key):
                pers[list_key] = _scrub_safe_value(pers[list_key])
        for text_key in ("travel_style", "budget_level"):
            if pers.get(text_key):
                pers[text_key] = scrub_contact_from_text(str(pers[text_key]))
        for internal_key in _GUEST_EVENT_REC_INTERNAL_PERSONALIZATION_KEYS:
            pers.pop(internal_key, None)
        out["personalization"] = pers
    stay_window = out.get("stay_window")
    if isinstance(stay_window, dict):
        sw = dict(stay_window)
        for sw_key in ("check_in", "check_out"):
            if sw.get(sw_key):
                sw[sw_key] = scrub_contact_from_text(str(sw[sw_key]))
        sw.pop("inferred", None)
        out["stay_window"] = sw
    sanitized: List[Dict[str, Any]] = []
    for rec in out.get("recommendations") or []:
        item = dict(rec)
        raw_id = item.get("id")
        for key in _GUEST_EVENT_REC_INTERNAL_REC_KEYS:
            item.pop(key, None)
        if raw_id is not None:
            item["event_id"] = scrub_contact_from_text(str(raw_id)) or str(raw_id)
        for text_key in (
            "title",
            "description",
            "admission_info",
            ...
            if item.get(text_key):
                item[text_key] = scrub_contact_from_text(
                    str(item[text_key]),
                    scrub_urls=text_key in {"description", "url"},
                )
        for list_key in ("cities", "regions"):
            if item.get(list_key):
                item[list_key] = _scrub_safe_value(item[list_key])
        if item.get("url"):
            item["url"] = scrub_contact_from_text(str(item["url"]), scrub_urls=True)
        sanitized.append(item)
    out["recommendations"] = sanitized
    return out
...
class EventRecommendationService:
    ...
    def _feed_row_to_candidate(
        ...
        candidate = EventCandidate(
            ...
            title=title,
            description=content,
            search_blob=_normalize_match_text(
                f"{title} {content} {' '.join(kw)} {row.get('event_type') or ''} {row.get('age_group') or ''}"
            ),
            keywords=kw,
            cities=cities,
            regions=regions,
            ...
            end_date=end_date,
            event_type=row.get("event_type") or row.get("content_type"),
    ...
    def _seasonal_to_candidate(self, ev: SeasonalEvent) -> Optional[EventCandidate]:
        ...
        if not _is_guest_visible_event(ev.name, guest_body or search_body):
            return None
        return EventCandidate(
            id=str(ev.id),
            source="seasonal",
            title=ev.name,
            description=guest_body or ev.description,
            search_blob=_normalize_match_text(
                f"{ev.name} {search_body} {ev.event_type} {ev.city}"
            ),
    ...
    def _preference_score(self, c: EventCandidate, ctx: GuestEventContext) -> float:
        ...
        if "family" in ctx.interests or "kids" in ctx.keywords:
            if c.age_group in ("family", "kids"):
                hit_scores.append(0.95)
            if any(t in blob for t in ("family", "kids")):
                hit_scores.append(0.85)

        if "wine" in ctx.interests or "food" in ctx.interests:
            if c.event_type == "food_wine":
                hit_scores.append(0.95)
            if any(t in blob for t in ("wine", "gastro", "vino")):
                hit_scores.append(0.85)
