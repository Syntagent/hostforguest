...
class EventSourceDiscoveryAgent:
    ...
    async def discover_for_location(
        self,
        city: str,
        *,
        ...
        """Discover event sources for a city (bot-triggered)."""
        if host:
            return await self.discover_for_host(host, profile, max_proposals=max_proposals)
        ...
        ctx = {
            ...
            "lng": None,
            "country": "Croatia",
            "existing_registry_urls": [d["url"] for d in load_national_event_sources()][:20],
        }
        raw = await self._call_llm(
            (
                f"Property city: {ctx.get('city')}\n"
                f"Region: {ctx.get('region')}\n"
                f"Already monitored URLs (do not duplicate): {ctx.get('existing_registry_urls')[:15]}\n"
                f"Propose up to {max_proposals} new official event listing sources."
            ),
            "system-discovery",
        )
        items = self._parse_proposals_json(raw)
        created_sources = await self._ensure_content_sources_from_items(items, ctx)
        return {
            "success": True,
            "context": ctx,
            "proposals_parsed": len(items),
            "sources_created": len(created_sources),
            "city": city,
        }

    async def _ensure_content_sources_from_items(
        ...
        created: List[ContentSource] = []

        for item in items:
            url = str(item.get("proposed_url") or "").strip()
            if not url or url in existing_urls or not url.startswith("http"):
                continue
            ...
            source = await scraper.create_content_source(
                ContentSourceCreate(
                    name=str(item.get("proposed_name") or url)[:200],
                    url=url[:500],
