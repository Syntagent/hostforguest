...
class EventsFeedService:
    ...
    def _local_event_to_dict(self, ev: LocalEvent) -> Dict[str, Any]:
        start = ev.start_at
        end = ev.end_at
        ...
        *,
        ...
        for ev in result.scalars().all():
            if is_site_chrome_title(ev.title or ""):
                continue
            out.append(self._local_event_to_dict(ev))
        return out
    ...
    async def get_updates(
        ...
        *,
        ...
        if content_types and ContentType.EVENTS not in content_types:
            return await self._get_content_updates(city=city, content_types=content_types, hours=hours, limit=limit)

        local = await self.get_local_events(city=city, hours=max(hours, 168), limit=limit)
        if local:
            return local
        return await self._get_content_updates(
            city=city, content_types=[ContentType.EVENTS], hours=hours, limit=limit
        )
