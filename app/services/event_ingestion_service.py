...
class EventIngestionService:
    ...
    async def _upsert_drafts(
        ...
        for draft in drafts:
            ...
            if existing:
                ...
                existing.language = draft.language or existing.language
                ...
                existing.lat = draft.lat if draft.lat is not None else existing.lat
                existing.lng = draft.lng if draft.lng is not None else existing.lng
                existing.event_type = draft.event_type
                existing.age_group = draft.age_group
                existing.price = draft.price
                existing.is_recurring = draft.is_recurring
    ...
    async def purge_pre_2026_events(self) -> Dict[str, Any]:
        """Remove stale events before 2026-01-01 (Gemma4 re-scrape cleanup)."""
        ...
        if rows:
            await self.db.commit()
        return {"deleted": len(rows), "cutoff": cutoff.isoformat()}

    async def ingest_for_city(self, city: str, *, region: Optional[str] = None) -> Dict[str, Any]:
        """Sync all registry sources matching city/region into local_events."""
        definitions = self._definitions_to_sync()
        city_l = city.strip().lower()
        matched = [
            d
            for d in definitions
            if (d.get("city") or "").strip().lower() == city_l
            or (region and (d.get("region") or "").strip().lower() == region.strip().lower())
        ]
        if not matched:
            matched = [d for d in definitions if city_l in (d.get("name") or "").lower()][:4]
        results = []
        for defn in matched:
            results.append(await self.sync_source(defn["slug"]))
        ok = sum(1 for r in results if r.get("success"))
        return {"city": city, "sources": len(results), "successful": ok, "results": results}
